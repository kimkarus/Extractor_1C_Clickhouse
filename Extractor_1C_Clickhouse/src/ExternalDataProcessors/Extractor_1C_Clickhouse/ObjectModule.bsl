...
Процедура ЗаписатьРезультатЗапросаЭкстракторНаСервере(Запрос, Параметры, ПараметрыЗапроса, ПериодДата,
	...
	//
	Если (Параметры.ТипБазы = "Local" И (ДополнительныеПараметры.ФорматДанных = "XLS" ИЛИ ДополнительныеПараметры.ФорматДанных = "XLSX"))
		ИЛИ Параметры.ТипБазы = "Google Sheets" Тогда
...
Функция ВыполнитьЗапросPOST(ТекстЗапроса, Параметры, ТаблицаЗначенийЗапроса = Неопределено)
	...
	ИначеЕсли Параметры.ТипБазы = "Google Sheets" Тогда
		РезультатВыполненияЗапроса = ВыполнитьЗапросPOST_GoogleSheets(ТекстЗапроса, Параметры, ТаблицаЗначенийЗапроса);
	ИначеЕсли Параметры.ТипБазы = "PostgreSQL" Тогда
...
Функция ВыполнитьЗапросPOST_GoogleSheets(ТекстЗапроса, Параметры, ТаблицаЗначенийЗапроса = Неопределено)
	Запрос = ТекстЗапроса;
	//
	СтрокаПериодДата = "";
	Если Параметры.ЕстьДатаВПараметрахЗапроса Тогда
...
Функция ПолучитьМассивФорматовДанныхПоТипуGoogleSheets()
	МассивРезультат = Новый Массив;
	МассивРезультат.Добавить("Добавление");
	МассивРезультат.Добавить("Замена");
	Возврат МассивРезультат;
КонецФункции
//<<%%>> ilya@kimkarus.ru @kimkarus_service_bot kimkarus.ru # Ilya Kosarev 26-08-2025 19:30 end
