...
Функция ЗаполнитьПараметры() Экспорт
	...
	СтруктураПараметры.Вставить("ТипБазы", "Local");
	СтруктураПараметры.Вставить("ФорматДанных", "XLS");
...
Процедура ВыгрузитьДанныеВПриемникНаСервере(Параметры, ТипКоманды = "") Экспорт
	//
	Параметры.Вставить("Local", "Local");
...
//		РезультатВыполненияЗапроса = ВыполнитьЗапросPOST(ТекстЗапросаPOST, Параметры, ВыборкаРезультатаСокр);
	...
	Иначе
		Если ДополнительныеПараметры.ОднимФайлом Тогда
			...
			//
			Если Параметры.ТипБазы = "Local" Тогда
