...
Процедура ВыполнитьКомандуСервер(ИдентификаторКоманды) Экспорт
	...
	Если МассивПодстроки[1] = "ClickHouseНаСервере" Тогда  
		...
		КонецЕсли;
		ВыгрузитьДанныеВПриемникНаСервере(ПараметрыОбработки, МассивПодстроки[2]);			
	ИначеЕсли МассивПодстроки[1] = "НастройкаЗапроса" Тогда
		...
		Если ПараметрыОбработкиСохраненные.НастройкиЗапросов.Количество() > 0 Тогда
			...
			Если ПараметрыОбработкиСохраненные.НастройкиЗапросов.Количество() >= НомерНастройки	Тогда
				...
				ВыгрузитьДанныеВПриемникНаСервере(ПараметрыОбработки, "НастройкаЗапроса");	
			КонецЕсли;
		КонецЕсли;
...
Функция ЗаполнитьПараметры() Экспорт
	...
	СтруктураПараметры.Вставить("ДополнительныеКлючи", "");
	СтруктураПараметры.Вставить("МассивДополнительныхКлючей", ПолучитьМассивДополнительныхКлючей());
	//
	СтруктураПараметры.Вставить("ВключитьАвтозапускЗапросовПриОткрытии", Ложь);
	СтруктураПараметры.Вставить("ВключитьАвтосохранениеПриЗакрытии", Истина);
	//
...
Процедура ВыгрузитьДанныеВПриемникНаСервере(Параметры, ТипКоманды = "") Экспорт
	...
	Если Параметры.ТипБазы = "Google Sheets" Тогда
		Параметры.Вставить("Адрес", "sheets.googleapis.com");
		Параметры.Вставить("ЭтоЗащищенноеСоединение", Истина);
		Параметры.Вставить("Порт", 443);
		ТекущийТокен = ПолучитьТокен_GoogleSheets(Параметры);
		Если ТекущийТокен = Неопределено Тогда
			Возврат;
		КонецЕсли;
		Параметры.Вставить("Пароль", ТекущийТокен);
		Если Не ЕстьТаблицаВ(Параметры.ИмяТаблицыТранслит, Параметры) Тогда
			ТаблицаСоздана = СоздатьНовыйЛист_GoogleSheets(Параметры);
			Если НЕ ТаблицаСоздана Тогда
				Возврат;
			КонецЕсли;
		КонецЕсли;
		Если Параметры.СпособОбновления = "Очистка" Тогда
			ТаблицаОчищена = ОчиститьЛист_GoogleSheets(Параметры);
			Если НЕ ТаблицаОчищена Тогда
				Возврат;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если Параметры.ТипБазы <> "REST"
		...
		Если Не ЕстьБазаВ(Параметры.ИмяБазы, Параметры) Тогда
			_ТекстЗапроса = ТекстЗапросаСозданиеБазыДанных(Параметры);
			БазаСоздана = ВыполнитьЗапросPOST(_ТекстЗапроса, Параметры);
		КонецЕсли;
	КонецЕсли;
	...
	Если Параметры.ТипБазы <> "REST"
		...
		И Параметры.ТипБазы <> "Google Sheets" Тогда
		Если Не ЕстьТаблицаВ(Параметры.ИмяТаблицыТранслит, Параметры) Тогда
			_ТекстЗапроса = ТекстЗапросаСозданиеТаблицыПериодСхемаКомпоновкиДанных(Параметры, ПараметрыЗапроса.ПараметрыМакета, ПараметрыЗапроса.ПоляНабораДанных, ПараметрыЗапроса.МассивПолейКлючейПериод, ПараметрыЗапроса.ДополнительныеПараметры);
	...
	Если Параметры.ТипБазы <> "REST"
		И Параметры.ТипБазы <> "Local"
		И Параметры.ТипБазы <> "Google Sheets" Тогда
		ОписаниеТаблицыБД = ОписаниеТаблицыВ(Параметры.ИмяТаблицыТранслит, Параметры);
...
Функция ДатаСУчетомПериодичности(_Дата, Периодичность, ЭтоНачало = Истина)
	...
	ИначеЕсли Периодичность = "Час" Тогда
		Если ЭтоНачало Тогда
			Возврат НачалоЧаса(_Дата);
		Иначе
			Возврат КонецЧаса(_Дата);
		КонецЕсли;
	Иначе
		Если ЭтоНачало Тогда
			Возврат НачалоМесяца(_Дата);
...
Функция ПолучитьСтруктуруПараметровЗапроса(Параметры) Экспорт
	...
	ДополнительныеПараметры.Вставить("ДополнительныеКлючи", Параметры.ДополнительныеКлючи);
...
Функция УстановитьПараметрыЗапросаПоУмолчанию(Параметры, ПараметрыЗапроса)
	...
	Если Не ЕстьТаблицаВ(Параметры.ИмяТаблицы, Параметры) Тогда
		_ТекстЗапроса = ТекстЗапросаСозданиеТаблицыПериодСхемаКомпоновкиДанных(Параметры, ПараметрыМакета, ПоляНабораДанных, МассивПолейКлючейПериод, ДополнительныеПараметры);
		Если Параметры.ТипБазы <> "REST"
			И Параметры.ТипБазы <> "Local"
			И Параметры.ТипБазы <> "Google Sheets" Тогда
			ТаблицаСоздана = ВыполнитьЗапросPOST(_ТекстЗапроса, Параметры);
...
Процедура ВыгрузитьДанныеВПриемник_Запрос_НаСервере(Запрос, Параметры, ПараметрыЗапроса)
	...
	КонецЕсли;
	//
	КоличествоСчетчик = ДополнительныеПараметры.КоличествоДнейНазад;
	Если ОсобыеПараметры.Периодичность = "Час" Тогда
		КоличествоСчетчик = КоличествоСчетчик * 24;
	КонецЕсли;
	//
	Если КоличествоСчетчик > 0 Тогда
		Параметры.Вставить("КоличествоЗапросов", КоличествоСчетчик + 1);
		НоваяДатаОтчета = НачалоДня(Параметры.ДатаНачала);
		Для Счетчик = 1 По КоличествоСчетчик + 1 Цикл
			//
			...
			//
			Если ОсобыеПараметры.Периодичность = "Месяц" Тогда
				СтараяДатаОтчета = НоваяДатаОтчета;
				НоваяДатаОтчета = ДатаМесяцВперед(НоваяДатаОтчета, 1);
				КоличествоДнейДобавитьКСчетчику = РозностьВДнях(НоваяДатаОтчета, СтараяДатаОтчета);
				Счетчик = Счетчик + КоличествоДнейДобавитьКСчетчику - 1;
			ИначеЕсли ОсобыеПараметры.Периодичность = "День" Тогда
				НоваяДатаОтчета = ДатаДнейВперед(НоваяДатаОтчета, 1);
			ИначеЕсли ОсобыеПараметры.Периодичность = "Час" Тогда
				НоваяДатаОтчета = ДатаЧасВперед(НоваяДатаОтчета, 1);
			Иначе
				НоваяДатаОтчета = ДатаДнейВперед(НоваяДатаОтчета, 1);
			КонецЕсли;
			//
			...
			//
			Если Параметры.ТипБазы = "Google Sheets" Тогда
				//ВыполнитьОжиданиеЦиклСекунд(Параметры.Таймаут);
				ВыполнитьОжиданиеHTTPСекунд(Параметры.Таймаут);
			КонецЕсли;
		КонецЦикла;
	Иначе
...
Процедура ЗаписатьРезультатЗапросаЭкстракторНаСервере(Запрос, Параметры, ПараметрыЗапроса, ПериодДата,
	...
	Если Параметры.ТипБазы = "REST" 
		...
		Если ДополнительныеПараметры.ЭтоШапкаПерваяСтрока Тогда
			...
			_ТекстЗапроса = _ТекстЗапроса + СтрокаПоля + "" + Символы.ПС;
		КонецЕсли;
	КонецЕсли;
	ИндексЦикла = 0;
	//
		...
		Если ДополнительныеПараметры.ОднимФайлом Тогда
			...
			ИначеЕсли Параметры.ТипБазы = "REST" Тогда
				Если ДополнительныеПараметры.ФорматДанных = "CSV_WH" Тогда
					//_ТекстЗапроса = _ТекстЗапроса + ЗначенияЗапроса + ";";
				...
				ИначеЕсли ДополнительныеПараметры.ФорматДанных = "PostgreSQL" Тогда
					_ТекстЗапроса = "INSERT INTO " + Параметры.ИмяТаблицыТранслит + "";
				ИначеЕсли ДополнительныеПараметры.ФорматДанных = "MySQL" Тогда
					_ТекстЗапроса = "INSERT INTO " + Параметры.ИмяБазы + "." + Параметры.ИмяТаблицыТранслит + "";
				Иначе
					//ТекстЗапросаPOST = _ТекстЗапроса + ЗначенияЗапроса + ";";
				КонецЕсли;
			...
			Для Каждого СтрокаР Из ВыборкаРезультата Цикл
				//Если ВернутьПроверкуИндексаЦикла(ИндексЦикла, ПолучитьКоличествоЦикловЗапрос()) Тогда
				//	ИндексЗаписи = КоличествоЗаписей - 1;
				//КонецЕсли;
				ИндексЦикла = ИндексЦикла + 1;
				...
				Если КоличествоСтрокОбработано = КоличествоСтрокМаксЗаРаз Или (ИндексЗаписи + 1) = КоличествоЗаписей Тогда
					Если Параметры.ТипБазы = "ClickHouseDB" Тогда
						...
						ТекстЗапросаPOST = ПолучитьТелоЗапросаClickHouseDB(_ТекстЗапроса, ЗначенияЗапроса);
					ИначеЕсли Параметры.ТипБазы = "REST" Тогда
						ТекстЗапросаPOST = _ТекстЗапроса + ВернутьЗначенияЗапросREST(СтрокаР, Параметры,
					...
					ИначеЕсли Параметры.ТипБазы = "PostgreSQL" Тогда
						ЗначенияЗапроса = ЗначенияЗапроса + ВернутьЗначенияЗапрос(СтрокаР, Параметры, ПериодДата,
							МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница);
						ТекстЗапросаPOST = ПолучитьТелоЗапросаPostgreSQL(МассивПолей, _ТекстЗапроса, ЗначенияЗапроса,
							ПараметрыЗапроса, МассивПолейЗначения);
					ИначеЕсли Параметры.ТипБазы = "MySQL" Тогда
						ЗначенияЗапроса = ЗначенияЗапроса + ВернутьЗначенияЗапрос(СтрокаР, Параметры, ПериодДата,
							МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница);
						ТекстЗапросаPOST = ПолучитьТелоЗапросаMySQL(МассивПолей, _ТекстЗапроса, ЗначенияЗапроса,
							МассивПолейЗначения);
					Иначе
					КонецЕсли;
					//
...
Функция ПолучитьТелоЗапросаPostgreSQL(_МассивПолей, _ТекстЗапроса, _ЗначенияЗапроса, _ПараметрыЗапроса, _МассивПолейЗначения)
	...
	//
	Если _МассивПолейЗначения.Количество() > 1 Тогда
...
Функция ВыполнитьЗапросPOST(_ТекстЗапроса, Параметры, ТаблицаЗначенийЗапроса = Неопределено, МассивПолей = Неопределено)
	...
	Если Параметры.ТипБазы = "ClickHouseDB" Тогда
		РезультатВыполненияЗапроса = ВыполнитьЗапросPOST_ClickHouseDB(_ТекстЗапроса, Параметры);
	ИначеЕсли Параметры.ТипБазы = "REST" Тогда
		РезультатВыполненияЗапроса = ВыполнитьЗапросPOST_REST(_ТекстЗапроса, Параметры);
	ИначеЕсли Параметры.ТипБазы = "Local" Тогда
		РезультатВыполненияЗапроса = ВыполнитьЗапросPOST_Local(_ТекстЗапроса, Параметры, ТаблицаЗначенийЗапроса);
	ИначеЕсли Параметры.ТипБазы = "Google Sheets" Тогда
		РезультатВыполненияЗапроса = ВыполнитьЗапросPOST_GoogleSheets(_ТекстЗапроса, Параметры, ТаблицаЗначенийЗапроса, МассивПолей);
	ИначеЕсли Параметры.ТипБазы = "PostgreSQL" Тогда
		РезультатВыполненияЗапроса = ВыполнитьЗапросPOST_PostgreSQL(_ТекстЗапроса, Параметры);
	ИначеЕсли Параметры.ТипБазы = "MySQL" Тогда
		РезультатВыполненияЗапроса = ВыполнитьЗапросPOST_MySQL(_ТекстЗапроса, Параметры);
	Иначе
	КонецЕсли;
...
Функция ВыполнитьРезультатВыполненияЗапросаПосле(Параметры)
	...
	Если Параметры.ТипБазы = "ClickHouseDB" Тогда
		_ТекстЗапроса = "OPTIMIZE TABLE " + Параметры.ИмяБазы + "." + Параметры.ИмяТаблицыТранслит +" FINAL";
		РезультатВыполненияЗапроса = ВыполнитьЗапросPOST_ClickHouseDB(_ТекстЗапроса + ";", Параметры);
	ИначеЕсли Параметры.ТипБазы = "PostgreSQL" Тогда
	ИначеЕсли Параметры.ТипБазы = "MySQL" Тогда
	Иначе
...
Функция ВернутьAlter(ТранслитСтрокаМ, КолонкаЗ, Параметры)
	...
	ИначеЕсли Параметры.ТипБазы = "PostgreSQL" Тогда
		Возврат "ALTER TABLE " + Параметры.ИмяТаблицыТранслит
			+ " ADD COLUMN " + ТранслитСтрокаМ + " " + ПолучитьТипПоляНабора(КолонкаЗ, Параметры) + ";";
	ИначеЕсли Параметры.ТипБазы = "MySQL" Тогда
		Возврат "ALTER TABLE " + Параметры.ИмяБазы + "." + Параметры.ИмяТаблицыТранслит
			+ " ADD COLUMN `" + ТранслитСтрокаМ + "` " + ПолучитьТипПоляНабора(КолонкаЗ, Параметры) + ";";
...
Функция ВернутьИндекациюПоСКДОписаниеТаблицыБД(ВходящаяТЗ, ЗапросКолонки, Параметры, ДополнительныеПараметры)
	...
	Для Каждого КолонкаЗ Из ЗапросКолонки Цикл
		...
		НоваяСтрокаТЗ.СозданиеПоля = ВернутьAlter(ТранслитСтрокаМ, КолонкаЗ, Параметры);
		Индекс = Индекс + 1;
	КонецЦикла;
	//
	Для каждого СтрокаМ из МассивПолейЗапрос Цикл
		НайденныеСтрокиТЗ = РезультатТЗ.НайтиСтроки(Новый Структура("ИмяСКД", СтрокаМ));
		КоличествоНайденныеСтрокиТЗ = НайденныеСтрокиТЗ.Количество();
		Если КоличествоНайденныеСтрокиТЗ <= 0 Тогда
			НоваяСтрокаТЗ = РезультатТЗ.Добавить();
			Если СтрокаМ <> "Период" Тогда
				НоваяСтрокаТЗ.ЕстьРазница = Истина;
				Результат.Вставить("ЕстьРазница", Истина);
			КонецЕсли;
			НоваяСтрокаТЗ.Имя = СтрокаМ;
			НоваяСтрокаТЗ.ИмяСКД = СтрокаМ;
		КонецЕсли;
	КонецЦикла;
	//
	РезультатТЗ.Сортировать("ИндексСКД");
	//
...
Функция ВернутьЗначенияЗапрос(СтрокаР, Параметры, ПериодДата, МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница)
	...
	Для каждого СтрокаТЗ из ИндексацияТЗ Цикл
			...
			Иначе
				Если НайтиПолейВМассиве(МассивПолей, СтрокаП) Тогда
					...
					ИначеЕсли ТипЗнч(СтрокаР[СтрокаП]) = Тип("УникальныйИдентификатор") Тогда
						ЗначениеПоля = СтрокаПустаяИлиКовычка + СокрЛП(Строка(СтрокаР[СтрокаП])) + СтрокаПустаяИлиКовычка;
					Иначе
		...
		ИначеЕсли ЗначениеПоля = "''" Тогда
			Если СтрНайти(ВРег(СтрокаТЗ.ИмяСКД), ВРег("Дата")) > 0 Тогда
				ЗначениеПоля = СтрокаПустаяИлиКовычка + Дата(1,1,1,0,0,0) + СтрокаПустаяИлиКовычка;
			Иначе
				ЗначениеПоля = СтрокаПустаяИлиКовычка + "0" + СтрокаПустаяИлиКовычка;
			КонецЕсли;
		Иначе
...
Функция ВернутьЗначенияЗапросREST(СтрокаР, Параметры, ДополнительныеПараметры, ПериодДата, МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница, ПараметрыЗапроса, МассивПолейЗначения, _ЗначенияЗапроса, ЭтоКонецВыборки = Ложь, ЕстьКолонкаПериодРегистратора = Ложь)
	...
	ИначеЕсли Параметры.ФорматДанных = "JSON" Тогда
		Если Параметры.ТипБазы = "Google Sheets" Тогда
			ЗначениеДляЗапроса = ВернутьЗначенияЗапросREST_JSON_МассивЗначенийПолейВСтроку(СтруктураПолей, СтруктураПолейЗапрос);
		Иначе
...
Функция ВернутьЗначенияЗапросREST_CSV(_СтруктураПолей, _СтруктураПолейЗапрос, _ЗначениеВКовычки = Истина)
	...
	Для Каждого Элемент из _СтруктураПолейЗапрос Цикл
		...
		Если ТипЗнч(ЭлементЗначение) = Тип("Дата") Тогда
			Если ВРег(ЭлементКлюч) = ВРег("ДатаОбновленияЗаписи") > 0 Тогда
				ЭлементЗначение = ТекДата;
			Иначе
				//
			КонецЕсли;
...
Функция ВернутьЗначенияЗапросREST_JSON_МассивЗначенийПолейВСтроку(_СтруктураПолей, _СтруктураПолейЗапрос, _ЗначениеВКовычки = Истина)
	ТекДата = ТекущаяДата();
	СтрокаПустаяИлиКовычка = ?(_ЗначениеВКовычки,"'", "");
	МассивЗначенийПолейВСтроку = "[";
	КоличествоПолей = _СтруктураПолейЗапрос.Количество();
	Индекс = 1;
	ЭтоМассив = Ложь;
	Если ТипЗнч(_СтруктураПолейЗапрос) = Тип("Массив") Тогда
		ЭтоМассив = Истина;
	КонецЕсли;
	Для Каждого Элемент из _СтруктураПолейЗапрос Цикл
		Если ЭтоМассив Тогда
			ЭлементЗначение = Элемент;
		Иначе
			ЭлементЗначение = Элемент.Значение;
		КонецЕсли;
		...
		Если ТипЗнч(ЭлементЗначение) = Тип("Дата") Тогда
			...
			Иначе
				//
			КонецЕсли;
			ЗначениеПоля = ПолучитьДатуВремяISO8601(ЭлементЗначение, Истина, "T");
		ИначеЕсли ТипЗнч(ЭлементЗначение) = Тип("Булево") Тогда
			ЗначениеПоля = Число(ЭлементЗначение);
			ЗначениеПоля = СтрЗаменить(ЗначениеПоля, ",", ".");
		ИначеЕсли ТипЗнч(ЭлементЗначение) = Тип("Число") Тогда
			ЗначениеПоля = ВернутьЧислоБезПробелов(ЭлементЗначение, Ложь, ".");
		ИначеЕсли ТипЗнч(ЭлементЗначение) = Тип("Строка") Тогда
			ЗначениеПоля = СокрЛП(Строка(ЭлементЗначение));
			ЗначениеПоля = СтрокаПустаяИлиКовычка + ЭкранированиеСпециальныхСимволов(ЗначениеПоля) + СтрокаПустаяИлиКовычка;
		ИначеЕсли ТипЗнч(ЭлементЗначение) = Тип("УникальныйИдентификатор") Тогда
		...
		//
		Если Индекс <= 1 Тогда
			МассивЗначенийПолейВСтроку = МассивЗначенийПолейВСтроку + ЗначениеПоля;
		Иначе
			МассивЗначенийПолейВСтроку = МассивЗначенийПолейВСтроку + "," + ЗначениеПоля;
		КонецЕсли;
		//
...
Функция ВставитьЗаписьВБД(СтрокаР, Параметры, ПериодДата, ПоляНабораДанных = Неопределено, МассивПолей = Неопределено, МассивПолейЗапрос = Неопределено, ИндексацияТЗ = Неопределено, ЕстьРазница = Неопределено)
	...
	ИначеЕсли Параметры.ТипБазы = "Google Sheets" Тогда
		ВставитьЗаписьВGoogleSheets_ПериодСхемаКомпоновкиДанных(СтрокаР, Параметры, ПериодДата, ПоляНабораДанных, МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница);
	ИначеЕсли Параметры.ТипБазы = "PostgreSQL" Тогда
...
Процедура ВставитьЗаписьВGoogleSheets_ПериодСхемаКомпоновкиДанных(СтрокаР, Параметры, ПериодДата, ПоляНабораДанных, МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница)
	...
	МассивСтрокЗначений.Добавить(
		ВернутьЗначенияЗапросREST(СтрокаР, Параметры,
							Неопределено, ПериодДата, МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница,
							Неопределено, Неопределено, Неопределено, Истина, Ложь));
...
Функция ВернутьЧислоБезПробелов(ЧислоИзСтроки = 0, ЭтоГод = Ложь, ЧРД = ".")
	...
	Иначе
		СтрокаЧисло = Формат(ЧислоИзСтроки, "ЧН=0; ЧГ=0; ЧРД="+ЧРД);
	КонецЕсли;
...
//<<%%>> ilya@extractor1c.ru @extractor1c_support_bot extractor1c.ru # Ilya Kosarev 05-01-2026 9:30 start	 
Функция ВернутьМассивПорядокПериодИзРезультатаЗапроса(РезультатЗапроса, ДополнительныеПараметры, ИмяПоля = "Поле")
	М = Новый Массив;
	//
	ЕстьДатаОбновленияЗаписи = Ложь;
	Для каждого Колонка из РезультатЗапроса.Колонки Цикл
		Если Колонка[ИмяПоля] = "ДатаОбновленияЗаписи" Тогда
			ЕстьДатаОбновленияЗаписи = Истина;
		КонецЕсли;
	КонецЦикла;
	//
	Если ДополнительныеПараметры.ТипБазы = "MySQL"
		И Не ЕстьДатаОбновленияЗаписи Тогда
	...
	Возврат М;
КонецФункции
//<<%%>> ilya@extractor1c.ru @extractor1c_support_bot extractor1c.ru # Ilya Kosarev 05-01-2026 9:30 end
...
Функция ВернутьМассивПорядокПериодИзКолонокЗапроса(ЗапросКолонки, ДополнительныеПараметры)
	...
	Для каждого Колонка из ЗапросКолонки Цикл
		Если Колонка.Имя = "ДатаОбновленияЗаписи" Тогда
			ЕстьДатаОбновленияЗаписи = Истина;
		КонецЕсли;
	КонецЦикла;
	//
...
Функция ВернутьМассивПорядокПериодИзНабораДанных(ПоляНабораДанных, ДополнительныеПараметры, ИмяПоля = "Поле")
	...
	Для каждого Поле из ПоляНабораДанных Цикл
		Если Поле[ИмяПоля] = "ДатаОбновленияЗаписи" Тогда
			ЕстьДатаОбновленияЗаписи = Истина;
		КонецЕсли;
	КонецЦикла;
	//
...
Функция ВернутьМассивПолейКлючейПериодИзНабораДанных(ПоляНабораДанных, ДополнительныеПараметры, ИмяПоля = "Поле")
	...
	Если ДополнительныеПараметры.ИспользоватьДополнительныеКлючи
		...
		МассивДополнительныеКлючи = ПолучитьМассивДополнительныхКлючей(ДополнительныеПараметры);
	КонецЕсли;
...
Функция ПолучитьМассивДополнительныхКлючей(ДополнительныеПараметры = Неопределено) Экспорт
	ДополнительныеПараметрыОбъекта = ЭтотОбъект;
	Если ДополнительныеПараметры <> Неопределено Тогда
		ДополнительныеПараметрыОбъекта = ДополнительныеПараметры;
	КонецЕсли;
	//
	МассивМ = Новый Массив;
	//
	Если СтрДлина(ДополнительныеПараметрыОбъекта.ДополнительныеКлючи) > 0 Тогда
		МассивМ = СтрРазделитьРасширенная(ДополнительныеПараметрыОбъекта.ДополнительныеКлючи, ";");	
	Иначе
...
Функция ТекстЗапросаСозданиеБазыДанных(Параметры)
	_ТекстЗапроса = "";
	Если Параметры.ТипБазы = "ClickHouseDB" Тогда
		_ТекстЗапроса = "CREATE DATABASE " + Параметры.ИмяБазы + " COMMENT 'The Metrics database';";
	ИначеЕсли Параметры.ТипБазы = "PostgreSQL" Тогда
		_ТекстЗапроса = "CREATE DATABASE " + Параметры.ИмяБазы + " COMMENT 'The Metrics database';";
	ИначеЕсли Параметры.ТипБазы = "MySQL" Тогда
		_ТекстЗапроса = "CREATE DATABASE " + Параметры.ИмяБазы + " COMMENT 'The Metrics database';";
...
Функция ТекстЗапросаСозданиеТаблицыПериодСхемаКомпоновкиДанных(Параметры, ПараметрыМакета, ПоляМакета, ПоляКлючи, ДополнительныеПараметры, ИмяПоляМ = "Поле")
	...
	Если Параметры.ТипБазы = "ClickHouseDB" Тогда
		_ТекстЗапроса = ТекстЗапросаСозданиеТаблицыПериодСхемаКомпоновкиДанныхClickHouseDB(Параметры, ПараметрыМакета, ПоляМакета, ПоляКлючи, ДополнительныеПараметры, ИмяПоляМ);
	ИначеЕсли Параметры.ТипБазы = "PostgreSQL" Тогда
		_ТекстЗапроса = ТекстЗапросаСозданиеТаблицыПериодСхемаКомпоновкиДанныхPostgreSQL(Параметры, ПараметрыМакета, ПоляМакета, ПоляКлючи, ДополнительныеПараметры, ИмяПоляМ);
	ИначеЕсли Параметры.ТипБазы = "MySQL" Тогда
		_ТекстЗапроса = ТекстЗапросаСозданиеТаблицыПериодСхемаКомпоновкиДанныхMySQL(Параметры, ПараметрыМакета, ПоляМакета, ПоляКлючи, ДополнительныеПараметры, ИмяПоляМ);
	Иначе
		_ТекстЗапроса = "";
	КонецЕсли;
...
Функция ТекстЗапросаСозданиеТаблицыПериодСхемаКомпоновкиДанныхClickHouseDB(Параметры, ПараметрыМакета, ПоляМакета, ПоляКлючи, ДополнительныеПараметры, ИмяПоляМ)
	...
	Если ДополнительныеПараметры.ИспользоватьПериодВКлюче Тогда
		_ТекстЗапроса = _ТекстЗапроса + Символы.ПС + " (" + Транслит("Период") + " Datetime,";	
	Иначе
		_ТекстЗапроса = _ТекстЗапроса + Символы.ПС + " (";
	КонецЕсли;
	Счетчик = 1;
	Количество = ПоляМакета.Количество();
	Для каждого ПолеМ из ПоляМакета Цикл
		Если Счетчик < Количество Тогда 
			_ТекстЗапроса = _ТекстЗапроса + Символы.ПС + " " + Транслит(ПолеМ[ИмяПоляМ]) + " " + ПолучитьТипПоляНабораClickHouseDB(ПолеМ) + ",";
		Иначе
			_ТекстЗапроса = _ТекстЗапроса + Символы.ПС + " " + Транслит(ПолеМ[ИмяПоляМ]) + " " + ПолучитьТипПоляНабораClickHouseDB(ПолеМ);
		КонецЕсли;
	...
	_ТекстЗапроса = _ТекстЗапроса + Символы.ПС + ")";
	_ТекстЗапроса = _ТекстЗапроса + Символы.ПС + "ENGINE = ReplacingMergeTree";
	_ТекстЗапроса = _ТекстЗапроса + Символы.ПС + "PRIMARY KEY (" + ПолучитьКлючИзМассива(ПоляКлючи) + ")";
...
Функция ТекстЗапросаСозданиеТаблицыПериодСхемаКомпоновкиДанныхPostgreSQL(Параметры, ПараметрыМакета, ПоляМакета, ПоляКлючи, ДополнительныеПараметры, ИмяПоляМ)
	...
	Если ДополнительныеПараметры.ИспользоватьПериодВКлюче Тогда
		_ТекстЗапроса = _ТекстЗапроса + Символы.ПС + " (" + Транслит("Период") + " timestamp without time zone,";	
	Иначе
		_ТекстЗапроса = _ТекстЗапроса + Символы.ПС + " (";
	КонецЕсли;
	Счетчик = 1;
	Количество = ПоляМакета.Количество();
	Для каждого ПолеМ из ПоляМакета Цикл
		_ТекстЗапроса = _ТекстЗапроса + Символы.ПС + " " + Транслит(ПолеМ[ИмяПоляМ]) + " " + ПолучитьТипПоляНабораPostgreSQL(ПолеМ) + ",";
	КонецЦикла;
	СтрокаИзМассивКлючей = ПолучитьКлючИзМассива(ПоляКлючи);
	_ТекстЗапроса = _ТекстЗапроса + Символы.ПС + "CONSTRAINT "+ Параметры.ИмяТаблицыТранслит +"_pkey PRIMARY KEY (" + СтрокаИзМассивКлючей + ")";
	_ТекстЗапроса = _ТекстЗапроса + Символы.ПС + ");";
	_ТекстЗапроса = _ТекстЗапроса + Символы.ПС + "CREATE UNIQUE INDEX UNIQUE_" + Параметры.ИмяТаблицыТранслит + "";
	_ТекстЗапроса = _ТекстЗапроса + " ON " + Параметры.ИмяТаблицыТранслит + "";
	_ТекстЗапроса = _ТекстЗапроса + " (" + СтрокаИзМассивКлючей + ");";
...
Функция ТекстЗапросаСозданиеТаблицыПериодСхемаКомпоновкиДанныхMySQL(Параметры, ПараметрыМакета, ПоляМакета, ПоляКлючи, ДополнительныеПараметры, ИмяПоляМ)
	...
	_ТекстЗапроса = _ТекстЗапроса + Символы.ПС + " (";
	Если ДополнительныеПараметры.ИспользоватьПериодВКлюче Тогда
		_ТекстЗапроса = _ТекстЗапроса + "`" + Транслит("Период") + "` datetime,";	
	КонецЕсли;
	Для каждого ПолеМ из ПоляМакета Цикл
		_ТекстЗапроса = _ТекстЗапроса + " `" + Транслит(ПолеМ[ИмяПоляМ]) + "` " + ПолучитьТипПоляНабораMySQL(ПолеМ) + ",";
	КонецЦикла;
	СтрокаИзМассивКлючей = ПолучитьКлючИзМассива(ПоляКлючи, "", 1100, 50);
	_ТекстЗапроса = _ТекстЗапроса + Символы.ПС + "CONSTRAINT "+ Параметры.ИмяТаблицыТранслит +"_pkey UNIQUE (" + СтрокаИзМассивКлючей + ")";
	_ТекстЗапроса = _ТекстЗапроса + Символы.ПС + ")";
	_ТекстЗапроса = _ТекстЗапроса + " ENGINE=InnoDB DEFAULT CHARSET=utf8;";
...
Функция ПолучитьТипПоляНабораPostgreSQL(ПолеНабора)
	Если ТипЗнч(ПолеНабора.ТипЗначения) = Тип("ОписаниеТипов") Тогда
		...
		ИмяПоля = ПолеНабора.Заголовок;
		ПолеПрав = Прав(ИмяПоля, 5);
		Если ПолеПрав = "Текст" Тогда
			СтрокаПолеНабораТипЗначения = "Текст";
		КонецЕсли;
		Если ИмяПоля = "ДатаОбновленияЗаписи" Тогда
			СтрокаПолеНабораТипЗначения = "Дата";
		КонецЕсли;
		...
		ИначеЕсли СтрокаПолеНабораТипЗначения = "Текст" Тогда
			Возврат "text DEFAULT ''";
...
Функция ВыполнитьЗапросPOST_REST(_ТекстЗапроса, Параметры)
	...
	Если Параметры.ФорматДанных = "JSON" Тогда
		HTTPЗапрос.УстановитьТелоИзСтроки(_ТекстЗапроса, КодировкаТекста.UTF8, ИспользованиеByteOrderMark.НеИспользовать);
	КонецЕсли;
...
//	ЧтениеJSON.УстановитьСтроку(ПолучитьСтрокуИзДвоичныхДанных(МакетПодписиДвоичный));
...
//	КонецЦикла;
	...
	Для каждого Элемент из Параметры Цикл
		ЭлементЗначение = Элемент.Значение;
		ЭлементКлюч = Элемент.Ключ;
		Если СтрНайти(ЭлементКлюч, "GoogleSheets_") > 0 Тогда
			ЭлементКлюч = СтрЗаменить(ЭлементКлюч, "GoogleSheets_", "");
			Если ЭлементКлюч = "private_key" Тогда
				ЭлементЗначение = СтрЗаменить(ЭлементЗначение, "\n", Символы.ПС);
			КонецЕсли;
			СтруктураСертификата.Вставить(ЭлементКлюч, ЭлементЗначение);
		КонецЕсли;
	КонецЦикла;
	//
	...
	СтруктураJSON.Вставить("alg", "RS256");
	СтруктураJSON.Вставить("typ", "JWT");
	ЗаписьJSON = Новый ЗаписьJSON;
	ЗаписьJSON.УстановитьСтроку(Новый ПараметрыЗаписиJSON(ПереносСтрокJSON.Нет));
	ЗаписатьJSON(ЗаписьJSON, СтруктураJSON);
	СтрокаJSONХедер = ЗаписьJSON.Закрыть();
	
	Хедер64 = Base64Url(ПолучитьДвоичныеДанныеИзСтроки(СтрокаJSONХедер));
	
	СтруктураJSON.Очистить();
	
	//iss 	The email address of the service account.
	СтруктураJSON.Вставить("iss", СтруктураСертификата.client_email);
	
	//scope 	A space-delimited list of the permissions that the application requests.
	//смотреть какой скоп нужен например: https://developers.google.com/sheets/api/guides/authorizing
	СтруктураJSON.Вставить("scope", "https://www.googleapis.com/auth/spreadsheets");
	...
	СтруктураJSON.Вставить("aud", "https://www.googleapis.com/oauth2/v4/token");
	...
	СтруктураJSON.Вставить("exp", СекундДо);
	
	//iat 	The time the assertion was issued, specified as seconds since 00:00:00 UTC, January 1, 1970.
	СтруктураJSON.Вставить("iat", СекундДо-60*60);
	
	ЗаписьJSON = Новый ЗаписьJSON;
	ЗаписьJSON.УстановитьСтроку(Новый ПараметрыЗаписиJSON(ПереносСтрокJSON.Нет));
	ЗаписатьJSON(ЗаписьJSON, СтруктураJSON);
	СтрокаJSONКлэймСет = ЗаписьJSON.Закрыть();
	
	КлэймСет64 = Base64Url(ПолучитьДвоичныеДанныеИзСтроки(СтрокаJSONКлэймСет));
	...
	ПодписьBase64 = Base64Url(ПодписьДвоичная);
	
	JWT = JWTдляСигнатуры + "." + ПодписьBase64;
	...
	Попытка
		СтруктураJSON = ПолучитьСтруктуруJSON(ТелоОтвета);
	Исключение
		Возврат Ложь;
	КонецПопытки;
	
	Если СтруктураJSON.Свойство("access_token") Тогда
		Возврат СтруктураJSON.access_token;
	Иначе
		Возврат Неопределено;
	КонецЕсли;
КонецФункции
...
Функция ОчиститьЛист_GoogleSheets(Параметры)
	...
	Параметры.Вставить("Адрес", СтруктураURI.Хост);
	//
	ПараметрыURI = Новый Структура("HTTPМетод, ПараметрыЛиста, ПараметрыЗапроса, ЭтоПолучитьКоличествоСтрок", "POST", "", ":clear", Ложь);
	Результат = ВыполнитьЗапросGET_GoogleSheets(СтруктураURI, Параметры, ПараметрыURI);
	//
...
Функция СоздатьНовыйЛист_GoogleSheets(Параметры)
	...
	Параметры.Вставить("Адрес", СтруктураURI.Хост);
	//
	JSONЗапрос = Новый Структура;
    JSONЗапрос.Вставить("requests", Новый Массив);
    
    ЗапросЛиста = Новый Структура;
    ЗапросЛиста.Вставить("addSheet", Новый Структура);
	//
	СвойстваЛиста = Новый Структура;
	СвойстваЛиста.Вставить("title", Параметры.ИмяТаблицыТранслит);
	//СвойстваЛиста.Вставить("index", 0);
	СвойстваЛиста.Вставить("gridProperties", Новый Структура);;
	СвойстваЛиста.gridProperties.Вставить("rowCount", 1000);
	СвойстваЛиста.gridProperties.Вставить("columnCount", 26);
	//
	ЗапросЛиста.addSheet.Вставить("properties", СвойстваЛиста);
    JSONЗапрос.requests.Добавить(ЗапросЛиста);
	//
	Соединение = ПолучитьHTTPСоединение(Параметры);
	//
	Заголовки = Новый Соответствие;
	...
	//
	СтруктураURI.Вставить("ПутьНаСервере", "/v4/spreadsheets/" + Параметры.КлючТаблицы);
	ПараметрыСтрока = ":batchUpdate";
	//ПараметрыСтрока = "";
	//
	HTTPЗапрос = Новый HTTPЗапрос(СтруктураURI.ПутьНаСервере + ПараметрыСтрока, Заголовки);
	//
	Если Параметры.ФорматДанных = "JSON" Тогда
		...
		СтрокаСтруктураJSON = ПолучитьТелоJSONИзСтруктуры(JSONЗапрос, ПараметрыJSON, Истина);
...
Функция ВыполнитьЗапросPOST_GoogleSheets(_ТекстЗапроса, Параметры, ТаблицаЗначенийЗапроса = Неопределено, МассивПолей)
	...
	Параметры.Вставить("Адрес", СтруктураURI.Хост);
	//
	ПараметрыURI = Новый Структура("HTTPМетод, ПараметрыЛиста, ПараметрыЗапроса, ЭтоПолучитьКоличествоСтрок", "GET", "!A:A", "?majorDimension=ROWS", Истина);
	КоличествоСтрокНаЛисте = ВыполнитьЗапросGET_GoogleSheets(СтруктураURI, Параметры, ПараметрыURI);
	//
	Если КоличествоСтрокНаЛисте > 1 Тогда
		НотацияЦелевойАдресНачало = НотацияА1(КоличествоСтрокНаЛисте + ?(КоличествоСтрокНаЛисте > 0, 1, 0), 1);
		МассивЗаписей = ТаблицаЗначенийЗапроса;
	Иначе
		СтрокаШапка = ВернутьЗначенияЗапросREST_JSON_МассивЗначенийПолейВСтроку(МассивПолей, МассивПолей);
		НотацияЦелевойАдресНачало = НотацияА1(1, 1);
		МассивЗаписей.Добавить(СтрокаШапка);
		Для каждого Элемент из ТаблицаЗначенийЗапроса Цикл
			МассивЗаписей.Добавить(Элемент);
		КонецЦикла;
	КонецЕсли;
	
	КоличествоСтрок = МассивЗаписей.Количество();
	КоличествоКолонок = МассивПолей.Количество() + 1;
	//
	НотацияЦелевойАдресКонец = НотацияА1(КоличествоСтрокНаЛисте + КоличествоСтрок + ?(КоличествоСтрокНаЛисте > 0, 1, 0), КоличествоКолонок);
	ЦелевойАдресА1 = Параметры.ИмяТаблицыТранслит + "!"+НотацияЦелевойАдресНачало+":"+НотацияЦелевойАдресКонец;
	//
	...
	СтруктураДляJSON.Вставить("values", МассивЗаписей);
	//
	Соединение = ПолучитьHTTPСоединение(Параметры);
	//
	Заголовки = Новый Соответствие;
	...
	Заголовки.Вставить("Authorization", "Bearer " + Параметры.Пароль);
	//
	СтруктураURI.Вставить("ПутьНаСервере", "/v4/spreadsheets/" + Параметры.КлючТаблицы + "/values/" + Параметры.ИмяТаблицыТранслит + "!"+НотацияЦелевойАдресНачало+":"+НотацияЦелевойАдресКонец);
	...
	Если Параметры.ФорматДанных = "JSON" Тогда
		...
		СтрокаСтруктураJSON = СтрЗаменить(СтрокаСтруктураJSON, "[""[", "[[");
		СтрокаСтруктураJSON = СтрЗаменить(СтрокаСтруктураJSON, "]""]", "]]");
		СтрокаСтруктураJSON = СтрЗаменить(СтрокаСтруктураJSON, "]"",""[", "],[");
		HTTPЗапрос.УстановитьТелоИзСтроки(СтрокаСтруктураJSON, КодировкаТекста.UTF8, ИспользованиеByteOrderMark.НеИспользовать);
	КонецЕсли;
...
Функция ВыполнитьЗапросGET_GoogleSheets(_СтруктураURI, Параметры, ПараметрыURI)
	...
	Заголовки.Вставить("Authorization", "Bearer " + Параметры.Пароль);
	//
	_СтруктураURI.Вставить("ПутьНаСервере", "/v4/spreadsheets/" + Параметры.КлючТаблицы + "/values/" + Параметры.ИмяТаблицыТранслит + ПараметрыURI.ПараметрыЛиста);
	//
	HTTPЗапрос = Новый HTTPЗапрос(_СтруктураURI.ПутьНаСервере + ПараметрыURI.ПараметрыЗапроса, Заголовки);
	//
	Результат = Соединение.ВызватьHTTPМетод(ПараметрыURI.HTTPМетод, HTTPЗапрос);
	ТекстОтвета = Результат.ПолучитьТелоКакСтроку();
	СтруктураСоответствийJSON = ПолучитьСтруктуруJSON(ТекстОтвета);
	//СтруктураСоответствийJSON = ПрочитатьJSON(ЧтениеJSON,, "ФункцияВосстановленияJSON",ФорматДатыJSON.ISO);
	Если ЭтоОшибкаЗапросаREST(ТекстОтвета) Тогда
		Возврат 0;
	КонецЕсли;
	Если Результат.КодСостояния = 200 Тогда
		Если ПараметрыURI.ЭтоПолучитьКоличествоСтрок Тогда
			Если СтруктураСоответствийJSON.Свойство("values") Тогда
				МассивСтрок = СтруктураСоответствийJSON.values;
...
Функция Base64Url(ДвоичныеДанные)
	...
	Если Прав(СтрокаBase64, 2) = "==" Тогда
		СтрокаBase64 = Лев(СтрокаBase64, СтрДлина(СтрокаBase64)-2);
		
	ИначеЕсли Прав(СтрокаBase64, 1) = "=" Тогда
...
Функция ДвоичныеИзЧисла(знач _Число)
	...
	Пока _Число > 0 Цикл
		ТекущийБайт = _Число%256;
		МассивЧиселБайт.Добавить(ТекущийБайт);
		_Число = (_Число - ТекущийБайт)/256;
	КонецЦикла;
	РазмерБайт = МассивЧиселБайт.Количество();
	Буфер = Новый БуферДвоичныхДанных(РазмерБайт);
	Для Индекс = 0 по РазмерБайт-1 Цикл
		Буфер.Установить(РазмерБайт - 1 - Индекс, МассивЧиселБайт[Индекс]);
	КонецЦикла;
...
Функция ВозведениеВСтепеньПоМодулю(Основание, Степень, Модуль)
	...
	Произведение = 1;
	Для Индекс = 0 по МассивЕдиниц.ВГраница() Цикл
		Произведение = Произведение * pow(МассивИзОснования[Индекс],МассивЕдиниц[Индекс]);
	КонецЦикла;
...
Процедура СохранитьТаблицуЗначенийВExcel(_ТаблицаЗначений, _ПутьДоФайла, _ТипФайла)
	...
	КонецЕсли;
	//
	ИмяКолонкиПоиск = "ДатаОбновленияЗаписи";
	ЕстьКолонкаИмяКолонкиПоиск = Ложь;
	Для каждого КолонкаТЗ из _ТаблицаЗначений.Колонки Цикл
		Если КолонкаТЗ.Имя = ИмяКолонкиПоиск Тогда
			ЕстьКолонкаИмяКолонкиПоиск = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	Если ЕстьКолонкаИмяКолонкиПоиск Тогда 
		ТекДата = ТекущаяДата();
		Для каждого СтрокаТЗ из _ТаблицаЗначений Цикл
			СтрокаТЗ[ИмяКолонкиПоиск] = ТекДата;	
		КонецЦикла;
	КонецЕсли;
	//
...
Функция ВыполнитьЗапросPOST_PostgreSQL(_ТекстЗапроса, Параметры)
	Запрос = _ТекстЗапроса;
	//
	ТекстОтвета = КомандаВыполнитьPostgreSQL(_ТекстЗапроса, Параметры, "post");
	КодРасшифровкиОтвета = ЭтоОшибкаЗапросаPostgreSQL(ТекстОтвета);
	Если КодРасшифровкиОтвета >= 0 Тогда
		Возврат Истина;
	Иначе
...
Функция ВыполнитьЗапросPOST_MySQL(_ТекстЗапроса, Параметры)
	Запрос = _ТекстЗапроса;
	//
	ТекстОтвета = КомандаВыполнитьMySQL(_ТекстЗапроса, Параметры, "post");
...
Функция КомандаВыполнитьPostgreSQL(_ТекстЗапроса, Параметры, Тип)
	...
	ПараметрыЗапуска = "";
	ПараметрыЗапускаСтруктура = СформироватьПараметрыЗапуска_ВыполнитьPostgreSQL(_ТекстЗапроса, Параметры, Тип);
...
Функция КомандаВыполнитьMySQL(_ТекстЗапроса, Параметры, Тип)
	...
	АбсолютныйПутьСкрипта = СохранитьСкриптPythonНаСервере(ИмяФункции);
	ПараметрыЗапуска = "";
	ПараметрыЗапускаСтруктура = СформироватьПараметрыЗапуска_ВыполнитьMySQL(_ТекстЗапроса, Параметры, Тип);
	Для Каждого КлючЗначение Из ПараметрыЗапускаСтруктура Цикл
		ПараметрыЗапуска =  ПараметрыЗапуска + " """ + КлючЗначение.Значение + """";
	КонецЦикла;
	КодВозврата = ЗапуститьПриложениеPythonНаСервере(ИмяФункции, ПараметрыЗапуска, Параметры.АбсолютныйПутьPython);
...
Функция СформироватьПараметрыЗапуска_ВыполнитьPostgreSQL(_ТекстЗапроса, Параметры, Тип)
	...
	//
	СохранитьТекстВТекстовыйФайл(_ТекстЗапроса, АбсолютныйПуть);
	//
	...
	ПараметрыЗапуска.Вставить("Порт", СтрЗаменить(Строка(Параметры.Порт), Символы.НПП,""));
	//
...
Функция ЕстьТаблицаВ(ИмяТаблицы, Параметры)
	...
	ИначеЕсли Параметры.ТипБазы = "Google Sheets" Тогда
		РезультатВыполненияЗапроса = ЕстьТаблицаВGoogleSheets(ИмяТаблицы, Параметры);
	ИначеЕсли Параметры.ТипБазы = "PostgreSQL" Тогда
...
Функция ЕстьТаблицаВClickHouseDB(ИмяТаблицы, Параметры)
	...
	//
	ТекстОтвета = ВыполнитьЗапросGET_ClickHouseDB(_ТекстЗапроса, Параметры);
...
Функция ЕстьТаблицаВREST(ИмяТаблицы, Параметры)
	...
	//
	Возврат Истина;
	ТекстОтвета = ВыполнитьЗапросGET_REST(_ТекстЗапроса, Параметры);
	Если ТекстОтвета = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	Если ЭтоОшибкаЗапросаREST(ТекстОтвета) Тогда
...
Функция ЕстьТаблицаВGoogleSheets(ИмяТаблицы, Параметры)
	СтруктураURI = СтруктураURI(Параметры.Адрес);
	Параметры.Вставить("Адрес", СтруктураURI.Хост);
	ПараметрыURI = Новый Структура("HTTPМетод, ПараметрыЛиста, ПараметрыЗапроса, ЭтоПолучитьКоличествоСтрок", "GET", "!A1", "", Ложь);
	РезультатURI = ВыполнитьЗапросGET_GoogleSheets(СтруктураURI, Параметры, ПараметрыURI);
	Если РезультатURI > 0 Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Функция ЕстьТаблицаВPostgreSQL(ИмяТаблицы, Параметры)
	...
	//
	ТекстОтвета = ВыполнитьЗапросGET_PostgreSQL(_ТекстЗапроса, Параметры, "get");
	КодРасшифровкиОтвета = ЭтоОшибкаЗапросаPostgreSQL(ТекстОтвета);
	Если КодРасшифровкиОтвета >= 0 Тогда
		Возврат Истина;
	Иначе
...
Функция ЕстьТаблицаВMySQL(ИмяТаблицы, Параметры)
	...
	//
	ТекстОтвета = ВыполнитьЗапросGET_MySQL(_ТекстЗапроса, Параметры, "get_json");
	КодРасшифровкиОтвета = ЭтоОшибкаЗапросаMySQL(ТекстОтвета);
	Если КодРасшифровкиОтвета >= 0 Тогда
		Возврат Истина;
	Иначе
...
Функция ОписаниеТаблицыВPostgreSQL(ИмяТаблицы, Параметры)
	...
	ТЗ = ПолучитьСтруктуруТЗДляОписаниеТаблицы();
	//
	ТекстОтвета = ВыполнитьЗапросGET_PostgreSQL(_ТекстЗапроса, Параметры, "get_json");
	КодРасшифровкиОтвета = ЭтоОшибкаЗапросаPostgreSQL(ТекстОтвета);
	Если КодРасшифровкиОтвета < 0 Тогда
		Возврат ТЗ;
	КонецЕсли;
...
Процедура ОтправитьЖурналРегистрацииВClickHouse(Параметры) Экспорт
	...
	//
	Если Не ЕстьТаблицаВ(Параметры.ИмяТаблицы, Параметры) Тогда
		_ТекстЗапроса = ТекстЗапросаСозданиеТаблицыПериодСхемаКомпоновкиДанных(Параметры, ПараметрыМакета, ПоляНабораДанных, МассивПолейКлючейПериод, ДополнительныеПараметры, "Имя");
		ТаблицаСоздана = ВыполнитьЗапросPOST(_ТекстЗапроса, Параметры);
	КонецЕсли;
	//
...
Функция ПолучитьМассивПериодичности() Экспорт
	...
	//РезультатМассив.Добавить("Минута");
	//РезультатМассив.Добавить("Час");
...
Функция ПолучитьМассивТиповПриемника() Экспорт
	...
	Результат.Добавить("PostgreSQL");
	Результат.Добавить("MySQL");
	Результат.Добавить("REST");
	Результат.Добавить("Local");
