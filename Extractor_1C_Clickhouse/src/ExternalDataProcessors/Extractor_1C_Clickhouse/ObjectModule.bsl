...
Функция СведенияОВнешнейОбработке() Экспорт 
	...
	РегистрационныеДанные.Вставить("Версия", "1.3.7");
...
Функция ЗаполнитьПараметры() Экспорт
	...
	//ClickHouseDB, PostgreSQ, MySQL, REST, Local
...
Процедура ВыгрузитьДанныеВБазуДанныхНаСервере(Параметры, ТипКоманды = "") Экспорт
	...
	Если Параметры.ТипБазы <> "REST"
		И Параметры.ТипБазы <> "Local" Тогда
...
Функция ПолучитьСтруктуруПараметровЗапроса(Параметры) Экспорт
	...
	Если ДополнительныеПараметры.ТипБазы = "REST"
		ИЛИ ДополнительныеПараметры.ТипБазы = "Local" Тогда
...
Процедура ЗаписатьРезультатЗапросаЭкстракторНаСервере(Запрос, Параметры, ПараметрыЗапроса, 
	ПериодДата, ПоляНабораДанных = Неопределено, МассивПолей = Неопределено, 
		...
		Иначе
			Для каждого СтрокаР из ВыборкаРезультата Цикл
				...
				Если КоличествоСтрокОбработано = КоличествоСтрокМаксЗаРаз ИЛИ (ИндексЗаписи + 1) = КоличествоЗаписей Тогда
					...
					ИначеЕсли Параметры.ТипБазы = "REST" Тогда
						ТекстЗапросаPOST = ТекстЗапроса + ВернутьЗначенияЗапросREST(СтрокаР, Параметры, ДополнительныеПараметры, ПериодДата, МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница, ПараметрыЗапроса, МассивПолейЗначения, ЗначенияЗапроса, Истина, ЕстьКолонкаПериодРегистратора);
					ИначеЕсли Параметры.ТипБазы = "Local" Тогда
						ТекстЗапросаPOST = ТекстЗапроса + ВернутьЗначенияЗапросREST(СтрокаР, Параметры, ДополнительныеПараметры, ПериодДата, МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница, ПараметрыЗапроса, МассивПолейЗначения, ЗначенияЗапроса, Истина, ЕстьКолонкаПериодРегистратора);
					ИначеЕсли Параметры.ТипБазы = "PostgreSQL" Тогда
						ЗначенияЗапроса = ЗначенияЗапроса + ВернутьЗначенияЗапрос(СтрокаР, Параметры, ПериодДата, МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница);
				...
				Иначе
					...
					Если Параметры.ТипБазы = "REST"
						ИЛИ Параметры.ТипБазы = "Local" Тогда
						ЗначенияЗапроса = ВернутьЗначенияЗапросREST(СтрокаР, Параметры, ДополнительныеПараметры, ПериодДата, МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница, ПараметрыЗапроса, МассивПолейЗначения, ЗначенияЗапроса, Ложь, ЕстьКолонкаПериодРегистратора);
					Иначе
						ЗначенияЗапроса = ЗначенияЗапроса + ВернутьЗначенияЗапрос(СтрокаР, Параметры, ПериодДата, МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница);
...
Функция ВыполнитьЗапросPOST(ТекстЗапроса, Параметры, ТаблицаЗначенийЗапроса = Неопределено)
...
Функция ВернутьЗначенияЗапросREST(СтрокаР, Параметры, ДополнительныеПараметры, ПериодДата, МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница, ПараметрыЗапроса, МассивПолейЗначения, _ЗначенияЗапроса, ЭтоКонецВыборки = Ложь, ЕстьКолонкаПериодРегистратора = Ложь)
	...
	Если Параметры.ФорматДанных = "CSV"
		...
		ТекстЗапросаПоля = _ЗначенияЗапроса + ВернутьЗначенияЗапросREST_CSV(СтруктураПолей, СтруктураПолейЗапрос, ДополнительныеПараметры.ЗначениеВКовычки) + ?(ЭтоКонецВыборки, "", Символы.ПС);
...
Функция ВыполнитьЗапросPOST_Local(ТекстЗапроса, Параметры, ТаблицаЗначенийЗапроса = Неопределено)
	Запрос = ТекстЗапроса;
	//
	СтрокаПериодДата = ПолучитьДатуISO8601(Параметры.ПериодДата, Ложь);
	...
	ИмяФайла = Параметры.ИмяТаблицыТранслит + "_" + СтрокаПериодДата + РасширениеФайла;
	//
	ФайлЗапроса = ПолучитьИмяВременногоФайла();
	ТекстовыйФайл = Новый ТекстовыйДокумент;
	ТекстовыйФайл.УстановитьТекст(Запрос);
...
Функция ПолучитьМассивТиповПриемника() Экспорт
	...
	//МассивТипов.Добавить("GoogleSheets");
	Возврат МассивТипов;
КонецФункции
...
Функция ПолучитьМассивФорматовДанныхПоТипу(ТипПриемника = "") Экспорт
	...
	ИначеЕсли ТипПриемника = "GoogleSheets" Тогда
