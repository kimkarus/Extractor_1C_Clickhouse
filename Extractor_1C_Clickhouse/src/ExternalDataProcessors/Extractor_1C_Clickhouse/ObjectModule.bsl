...
Функция ВыполнитьЗапросPOST_GoogleSheets(ТекстЗапроса, Параметры, ТаблицаЗначенийЗапроса = Неопределено, МассивПолей)
	...
	Если СтруктураJSON.Свойство("access_token") Тогда
		Параметры.Вставить("Пароль", СтруктураJSON.access_token);
	Иначе
		Возврат Ложь;
	КонецЕсли;
	...
	Параметры.Вставить("Адрес", СтруктураURI.Хост);
	//
	КоличествоСтрокНаЛисте = ВыполнитьЗапросGET_GoogleSheets_ROWS(СтруктураURI, Параметры);
	//
	...
	Иначе
		//ТаблицаЗначенийЗапроса - добавляем первую строку с заголовкамиэ
...
Функция ВыполнитьЗапросGET_GoogleSheets_ROWS(_СтруктураURI, Параметры)
	Соединение = ПолучитьHTTPСоединение(Параметры);
	Заголовки = Новый Соответствие;
	Заголовки.Вставить("Content-Type", "application/json; charset=UTF-8");
	Заголовки.Вставить("Authorization", "Bearer " + Параметры.Пароль);
	//
	//СтруктураURI.Вставить("ПутьНаСервере", "/v4/spreadsheets/" + Параметры.КлючТаблицы + "/values/" + Параметры.ИмяТаблицыТранслит + "!"+НотацияЦелевойАдресНачало+":"+НотацияЦелевойАдресКонец);
	_СтруктураURI.Вставить("ПутьНаСервере", "/v4/spreadsheets/" + Параметры.КлючТаблицы + "/values/" + Параметры.ИмяТаблицыТранслит + "!A:A");
	//_СтруктураURI.Вставить("ПутьНаСервере", "/v4/spreadsheets/" + Параметры.КлючТаблицы);
	ПараметрыСтрока = "?majorDimension=ROWS";
	//ПараметрыСтрока = "?fields=sheets(properties(sheetId,title,gridProperties(rowCount,columnCount)))";
	//
	HTTPЗапрос = Новый HTTPЗапрос(_СтруктураURI.ПутьНаСервере + ПараметрыСтрока, Заголовки);
	//
