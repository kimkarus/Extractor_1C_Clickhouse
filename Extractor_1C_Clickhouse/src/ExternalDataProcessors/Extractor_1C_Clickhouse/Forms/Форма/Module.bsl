...
Процедура НастройкиЗапросовПриАктивизацииСтроки(Элемент)
	Если Объект.НастройкиЗапросов.Количество() > 0 Тогда
		...
		УстаноитьПериодИзНастройкиЗапроса(ТекущаяСтрока);
	КонецЕсли;
...
&НаКлиенте
Процедура УстаноитьПериодИзНастройкиЗапроса(ТекущаяСтрока)
	Если ТекущаяСтрока.ДатаНачала > Дата(1, 1, 1) Тогда
		Период.ДатаНачала = ТекущаяСтрока.ДатаНачала;			
	КонецЕсли;
	Если ТекущаяСтрока.ДатаОкончания > Дата(1, 1, 1) Тогда
		Период.ДатаОкончания = ТекущаяСтрока.ДатаОкончания;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПриОткрытии(Отказ)
	...
	//
	Если Объект.ВключитьАвтозапускЗапросовПриОткрытии Тогда
		ВыполнитьАвтозапускЗапросовПриОткрытии();		
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ВыполнитьАвтозапускЗапросовПриОткрытии()
	Если Объект.НастройкиЗапросов.Количество() <= 0 Тогда
		Возврат;
	КонецЕсли;
	//
	Для каждого СтрокаТЧ из Объект.НастройкиЗапросов Цикл
		ЗаполнитьЗначенияСвойств(Объект, СтрокаТЧ);
		УстаноитьПериодИзНастройкиЗапроса(СтрокаТЧ);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Объект.ВключитьАвтосохранениеПриЗакрытии Тогда
