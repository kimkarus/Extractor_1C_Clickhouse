...
Функция СведенияОВнешнейОбработке() Экспорт 
	...
	РегистрационныеДанные.Вставить("Версия", "1.1.4");
...
Процедура ВыгрузитьДанныеClickHouseНаСервере(Параметры, МассивПодстроки = Неопределено) Экспорт
	...
	Если Параметры.ЭтоТекущаяДата Тогда
		ТекущаяДата = ТекущаяДата();
		Параметры.Вставить("ДатаНачала", ДатаДнейНазад(ТекущаяДата, 1));
	...
	Если Не ЕстьТаблицаВClickHouse(Параметры.ИмяТаблицы, Параметры) Тогда
		ТекстЗапроса = ТекстЗапросаСозданиеТаблицыПериодСхемаКомпоновкиДанных(Параметры, ПараметрыЗапроса.ПараметрыМакета, ПараметрыЗапроса.ПоляНабораДанных, ПараметрыЗапроса.МассивПолейКлючейПериод, ПараметрыЗапроса.ДополнитеьныеПараметры);
...
Функция ПолучитьСтруктуруПараметровЗапроса(Параметры)
	...
	//
	ДополнитеьныеПараметры.Вставить("ИспользоватьПериодВКлюче", ПолучитьПараметрМакетаИПараметровПоИмени("ИспользоватьПериодВКлюче", ПараметрыМакета, Параметры));
	ДополнитеьныеПараметры.Вставить("КоличествоВПортиции", ПолучитьПараметрМакетаИПараметровПоИмени("КоличествоВПортиции", ПараметрыМакета, Параметры));
	ДополнитеьныеПараметры.Вставить("ОднимФайлом", ПолучитьПараметрМакетаИПараметровПоИмени("ОднимФайлом", ПараметрыМакета, Параметры));
	ДополнитеьныеПараметры.Вставить("Периодичность", ПолучитьПараметрМакетаИПараметровПоИмени("Периодичность", ПараметрыМакета, Параметры));
	//
	МассивПолей = ВернутьМассивПорядокПериодИзНабораДанных(ПоляНабораДанных, ДополнитеьныеПараметры);		  
	МассивПолейКлючейПериод = ВернутьМассивПолейКлючейПериодИзНабораДанных(ПоляНабораДанных, ДополнитеьныеПараметры);
	...
	Иначе
		ДополнитеьныеПараметры.Вставить("КоличествоДнейНазад", ПолучитьПараметрМакетаИПараметровПоИмени("КоличествоДнейНазад", ПараметрыМакета, Параметры));
	КонецЕсли;
...
Функция УстановитьПараметрыЗапросаПоУмолчанию(Параметры, ПараметрыЗапроса)
	...
	//
	Если Не ЕстьТаблицаВClickHouse(Параметры.ИмяТаблицы, Параметры) Тогда
		ТекстЗапроса = ТекстЗапросаСозданиеТаблицыПериодСхемаКомпоновкиДанных(Параметры, ПараметрыМакета, ПоляНабораДанных, МассивПолейКлючейПериод, ДополнитеьныеПараметры);
		ТаблицаСоздана = ВыполнитьЗапросPOST_ClickHouse(ТекстЗапроса, Параметры);
	КонецЕсли;
	//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 16-05-2024 9:30 start
...
Процедура ЗаписатьРезультатЗапросаClickHouseНаСервере(Запрос, Параметры, ПериодДата, ПоляНабораДанных = Неопределено, МассивПолей = Неопределено, ДополнитеьныеПараметры = Неопределено, ВходнаяТЗ = Неопределено, МассивПолейЗапрос = Неопределено, ИндексацияТЗ = Неопределено, ЕстьРазница = Неопределено)
	...
	Если ВходнаяТЗ = Неопределено Тогда
		...
		МассивПолейЗапрос = ВернутьМассивПорядокПериодИзРезультатаЗапроса(РезультатЗапроса, ДополнитеьныеПараметры);
...
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 01-11-2023 9:30 start	 
Функция ВернутьМассивПорядокПериодИзРезультатаЗапроса(РезультатЗапроса, ДополнитеьныеПараметры)
	М = Новый Массив;
	Если ДополнитеьныеПараметры.ИспользоватьПериодВКлюче Тогда
		ИмяПериод = "Период"; 	
...
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 03-11-2023 9:30 start
Функция ВернутьМассивПорядокПериодИзНабораДанных(ПоляНабораДанных, ДополнитеьныеПараметры)
	М = Новый Массив;
	Если ДополнитеьныеПараметры.ИспользоватьПериодВКлюче Тогда
		ИмяПериод = "Период"; 	
		М.Добавить(ИмяПериод);
...
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 03-11-2023 9:30 start			 
Функция ВернутьМассивПолейКлючейПериодИзНабораДанных(ПоляНабораДанных, ДополнитеьныеПараметры)
	М = Новый Массив;
	Если ДополнитеьныеПараметры.ИспользоватьПериодВКлюче Тогда
		ИмяПериод = "Период"; 	
		М.Добавить(ИмяПериод);
...
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 01-11-2023 9:30 start									   
Функция ТекстЗапросаСозданиеТаблицыПериодСхемаКомпоновкиДанных(Параметры, ПараметрыМакета, ПоляМакета, ПоляКлючи, ДополнитеьныеПараметры)
