...
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 27-10-2023 9:30 start																						 
Процедура ЗаписатьРезультатЗапросаClickHouseНаСервере(Запрос, Параметры, ПериодДата, ПоляНабораДанных = Неопределено, МассивПолей = Неопределено, ДополнитеьныеПараметры = Неопределено, ВходнаяТЗ = Неопределено, МассивПолейЗапрос = Неопределено) Экспорт
	//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 16-01-2023 18:00 start
	...
	Иначе
		...
		Для каждого СтрокаР из ВыборкаРезультата Цикл
			...
			Если Счетчик >= КоличествоСтрокМаксЗаРаз Тогда
				...
				//
				ЗапуститьФоновоеЗаданиеClickHouse(Запрос, Параметры, ПериодДата, ПоляНабораДанных, МассивПолей, ДополнитеьныеПараметры, ТЗ, МассивПолейЗапрос);
...
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 16-01-2023 18:00 start
Процедура ЗапуститьФоновоеЗаданиеClickHouse(Запрос, Параметры, ПериодДата, ПоляНабораДанных = Неопределено, МассивПолей = Неопределено, ДополнитеьныеПараметры = Неопределено, ВходнаяТЗ = Неопределено, МассивПолейЗапрос = Неопределено)
	ДополнитеьныеПараметры.Вставить("ОднимФайлом", Истина);
	УникальныйИдентификатор = Новый УникальныйИдентификатор;
	//
	ПараметрыЗапуска = Новый Структура;
	ПараметрыЗапуска.Вставить("Запрос", Запрос);
	ПараметрыЗапуска.Вставить("Параметры", Параметры);
	ПараметрыЗапуска.Вставить("ПериодДата", ПериодДата);
	ПараметрыЗапуска.Вставить("ПоляНабораДанных", ПоляНабораДанных);
...
Функция ВыполнитьФоновоеЗаданиеНаСервере(ПараметрыЗапуска, УникальныйИдентификатор)
    ...
    НаименованиеЗадания = НСтр("ru = 'Фоновое задание длительной: Синхронизация1C_ClickHouse");
    
    ВыполняемыйМетод = "ДлительныеОперации.ВыполнитьПроцедуруМодуляОбъектаОбработки";
    
    ПараметрыЗадания = Новый Структура;
	ПараметрыЗадания.Вставить("ИмяОбработки", ЭтотОбъект.ИспользуемоеИмяФайла);
    ПараметрыЗадания.Вставить("ИмяМетода", "ЗаписатьРезультатЗапросаClickHouseНаСервере");
