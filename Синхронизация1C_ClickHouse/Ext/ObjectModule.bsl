...
Функция ПолучитьОсобыеПараметры(ДополнитеьныеПараметры, ПараметрыМакета)
	СтруктураПараметров = Новый Структура("Передичность, ЕстьОсобоеПолеДатыОкончениеПериода");
	
	СтруктураПараметров.Вставить("Периодичность", ДополнитеьныеПараметры.Периодичность);
...
Функция УстановитьПараметрыЗапросаПоУмолчанию(Параметры, ПараметрыЗапроса)
	...
	КонецЕсли;
	//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 16-05-2024 9:30 start
	ОсобыеПараметры = ПолучитьОсобыеПараметры(ДополнитеьныеПараметры, ПараметрыМакета);
	//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 16-05-2024 9:30 end
	Для каждого СтрокаПараметра из ПараметрыМакета Цикл
...
Процедура ВыгрузитьДанныеClickHouse_Запрос_НаСервере(Запрос, Параметры, ПараметрыЗапроса)
	...
	ДополнитеьныеПараметры = ПараметрыЗапроса.ДополнитеьныеПараметры;
	//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 16-05-2024 9:30 start
	ДополнитеьныеПараметры.Вставить("ИмяТаблицы", Параметры.ИмяТаблицы);
	ОсобыеПараметры = ПолучитьОсобыеПараметры(ДополнитеьныеПараметры, ПараметрыМакета);
	//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 16-05-2024 9:30 end
	...
	Если ДополнитеьныеПараметры.ОднимФайлом Тогда
		...
		Для каждого СтрокаР из ВыборкаРезультата Цикл
			...
			Если КоличествоСтрокОбработано = КоличествоСтрокМаксЗаРаз ИЛИ ИндексЗаписи = КоличествоЗаписей Тогда
				ЗначенияЗапроса = ЗначенияЗапроса + (СтрокаР, Параметры, ПериодДата, МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница);
...
Процедура ЗапуститьФоновоеЗаданиеClickHouse(Параметры, ПериодДата, МассивПолей = Неопределено, ДополнитеьныеПараметры = Неопределено, ВходнаяТЗ = Неопределено, МассивПолейЗапрос = Неопределено, ИндексацияТЗ = Неопределено, ЕстьРазница = Неопределено)
	...
	ПараметрыЗапуска.Вставить("ИмяТаблицы", ДополнитеьныеПараметры.ИмяТаблицы);
