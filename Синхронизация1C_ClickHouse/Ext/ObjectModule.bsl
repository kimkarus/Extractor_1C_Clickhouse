...
Функция СведенияОВнешнейОбработке() Экспорт 
	...
	ДобавитьКоманду(ТаблицаКоманд, "БросаниеВагоновТип фоновое", "Синхронизация1C_ClickHouse_БросаниеВагоновТип" , "ВызовСерверногоМетода");
	ДобавитьКоманду(ТаблицаКоманд, "ОперативныйОтчетЛогисты фоновое", "Синхронизация1C_ClickHouse_ОперативныйОтчетЛогисты" , "ВызовСерверногоМетода");
...
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 01-11-2023 9:30 start
Функция ВернутьДату_ОперативныйОтчетЛогисты_НаСервере(ИмяПоля, ПериодОтчета, НоваяДатаОтчета)
...
Процедура ВыгрузитьДанныеClickHouse_ОперативныйОтчетЛогисты_НаСервере(Параметры)
	...
	Если Не ЕстьТаблицаВClickHouse(Параметры.ИмяТаблицы, Параметры) Тогда
		ТекстЗапроса = ТекстЗапросаСозданиеТаблицыПериодСхемаКомпоновкиДанных(Параметры, ПараметрыМакета, ПоляНабораДанных, ПолучитьКлючиClickHouse_ОперативныйОтчетЛогисты());
		ТаблицаСоздана = ВыполнитьЗапросPOST_ClickHouse(ТекстЗапроса, Параметры);
	КонецЕсли;
	//
	Для каждого СтрокаПараметра из ПараметрыМакета Цикл
		Если ТипЗнч(СтрокаПараметра) = Тип("Дата") Тогда
			Запрос.УстановитьПараметр(СтрокаПараметра.Имя, ВернутьДату_ОперативныйОтчетЛогисты_НаСервере(СтрокаПараметра.Имя, Параметры.ДатаНачала, Параметры.ДатаНачала));
		Иначе
	...
	Если НачалоДня(Параметры.ДатаОкончания) <> НачалоДня(Параметры.ДатаНачала) Тогда
		...
		Для Счетчик = 1 По КоличествоДней + 1 Цикл
			...
			Для каждого СтрокаПараметра из ПараметрыМакета Цикл
				Если ТипЗнч(СтрокаПараметра) = Тип("Дата") Тогда
					Запрос.УстановитьПараметр(СтрокаПараметра.Имя, ВернутьДату_ОперативныйОтчетЛогисты_НаСервере(СтрокаПараметра.Имя, Параметры.ДатаНачала, НоваяДатаОтчета));
				КонецЕсли;
			КонецЦикла;
